...
from modules.model import AGRELh, AGRELg
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, help='Name of the dataset for experiment.', default=f'prime_{current_time}')
    parser.add_argument('--input_dir', type=str, help='Input Directory of the parser', default='./dataset/')
    ...
    model_dir = os.path.join('models-prime', args.dataset)
    ...
    processed_data_path = os.path.join(input_dir, 'prime_tk.bin')
    logging.info('#' * 100)
    if True and os.path.exists(processed_data_path):
        ...
        dataset = DataSet(train_src=os.path.join( '../cpg/devign_data/sj2/sard-train.json'),
                          valid_src=os.path.join( '../cpg/devign_data/sj2/sard-valid.json'),
                          test_src=os.path.join( '../cpg/devign_data/sj2/sard-test.json'),
    ...
    if args.model_type == 'ggnn':
        ...
        model = AGRELg(input_dim=dataset.feature_size, output_dim=100,
                            num_steps=args.num_steps, max_edge_types=dataset.max_edge_type)
    ...
    train(model=model, dataset=dataset, epoches=100, dev_every=len(dataset.train_batches),
          ...
          save_path=model_dir + '/DevignModel', max_patience=20, log_every=5)  
